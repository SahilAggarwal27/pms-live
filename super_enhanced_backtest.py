"""
PMS SUPER-ENHANCED BACKTEST — L1-L5 + L6 + L10 (user-selected 7 layers)
...
Super layers ENABLED:
  L6: INDIA VIX regime — reduce size at VIX>25, cash at VIX>40 ✓
  L10: SECTOR ROTATION — overweight top-3 relative-strength sectors ✓

Super layers DISABLED (user preference):
  L7: Market breadth filter (BREADTH_MIN=0)
  L9: Kelly-optimal sizing (KELLY_FRAC=0 → equal weight)

Expected: 25% (Honest) → 32% (Enhanced) → 36-40% (L1-5+L6+L10)
"""
...
# L7 DISABLED (breadth check off — user preference)
BREADTH_MIN = 0.0

# L9 DISABLED (Kelly off — equal-weight sizing — user preference)
KELLY_FRAC = 0.0  # 0 = pure equal weight

# L10 Sector rotation strength
SECTOR_ROT_BONUS = 0.10  # top-3 sector bonus to score
SECTOR_ROT_PENALTY = 0.05  # bottom-3 penalty
...
def download_all():
    ...
    if vix is not None:
        data["_VIX"] = vix
        print(f"India VIX: {len(vix)} rows (from {vix.index[0].strftime('%Y-%m')})")
    else:
        print("India VIX unavailable - L6 defaults to full sizing pre-2008")
    return data
...
def sector_strength(data, date, lookback_days=126):
    """L10: rank sectors by relative strength"""
    sector_returns = {}
...
def rank_super(data, date_str, top_sectors):
    ...
    top_set = set([s for s,_ in top_sectors[:3]])
    bot_set = set([s for s,_ in top_sectors[-3:]]) if len(top_sectors)>=6 else set()
    for ticker, closes in data.items():
        ...
        try:
            ...
            base = 0.25*r3 + 0.50*r6 + 0.25*r12
            sec = sector_of(ticker)
            bonus = 0
            if sec in top_set: bonus = SECTOR_ROT_BONUS
            elif sec in bot_set: bonus = -SECTOR_ROT_PENALTY
            score = base + bonus
            ranked.append({"ticker":ticker,"cmp":round(cmp,2),"score":score,"base":base,
...
def run_backtest():
    print("="*70)
    print("PMS SUPER-ENHANCED BACKTEST — L1-L5 + L6 VIX + L10 Sector Rotation")
    print("(L7 breadth OFF, L9 Kelly OFF per user preference — cleaner strategy)")
    print("="*70)
    ...
    for i, date_str in enumerate(month_ends):
        ...
        regime_ok = nifty_ok and vix_mult > 0
        ...
        for t in to_sell:
            ...
            reason = "TRAIL" if t in forced_sells else ("NO_NIFTY" if not nifty_ok else ("VIX_PANIC" if vix_mult==0 else "ROTATION"))
        ...
        if regime_ok and picked:
            ...
            # L6 VIX sizing: reduce total allocation, but equal-weight per stock (L9 off)
        ...
        monthly_journal.append({
            ...
            "regimeOn":regime_ok,"niftyOk":nifty_ok,"vixMult":vix_mult,
        ...
        if (i+1)%12 == 0:
            top3 = [s for s,_ in top_sectors[:3]]
            print(f"  {date_str[:4]}: NAV Rs {final_nav/10_000_000:.2f}Cr | Tot {total:+.1f}% | VIX-mult {vix_mult} | Top sectors: {top3}")
    ...
    print("\n"+"="*70)
    print("SUPER-ENHANCED BACKTEST COMPLETE — L1-L5 + L6 + L10")
    print("="*70)
    print(f"CAGR:              {cagr:+.2f}%   (Honest 25.65%, target 36-40%)")
    ...
    print(f"Nifty-off months:  {regime_offs} | VIX-reduced months: {vix_offs}")
    print("="*70)
    ...
    output = {
        ...
        "summary":{
            ...
            "mode":"super-enhanced-L6-L10","exitReasons":reason_counts,
            "layers":"L1 Nifty + L2 15% Trail + L3 60% Vol + L4 30% Sector + L5 MultiTF Mom + L6 VIX regime + L10 Sector Rotation",
            "assumptions":"L1-L5 + L6 VIX + L10 Sector Rotation on 261 stock universe"
        },
