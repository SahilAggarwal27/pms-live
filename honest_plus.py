"""
PMS HONEST+ (V2-Light) — Honest strategy + 2 targeted additions

Base: honest_backtest.py logic (261-stock universe, historical costs, 3yr listing)
Adds:
  L2: 15% Trailing Stop  — cut losers early
  L6: India VIX Regime   — half size at VIX>25, cash at VIX>40

Skips overfitting-prone layers:
  ✗ L11 Dynamic universe (never triggered in v3)
  ✗ L12 Breadth filter   (killed bull years in v3)
  ✗ L13 50+200 DMA       (whipsawed in v3)
  ✗ L14 Cash buffer      (missed rallies in v3)
  ✗ L15 Tight sector cap (over-restrictive in v3)
  ✗ L16 Relative momentum (marginal)

Expected: Honest 25.65% → Honest+ 28-30% CAGR with lower max DD
"""
...
# Same 261-stock universe as honest_backtest.py
...
def rank_honest(data, date_str):
    """Same Honest ranking - 3yr listing filter, above 200-DMA, momentum cap"""
    ranked = []
...
def run_backtest():
    print("="*70)
    print("PMS HONEST+ (V2-Light) — Honest baseline + L2 Trail + L6 VIX only")
    print("="*70)
    ...
    for i, date_str in enumerate(month_ends):
        ...
        # L2: Update peaks and check trailing stops
        forced_sells = []
        ...
        picked = rank_honest(data, date_str)
        picked_tickers = [p["ticker"] for p in picked]

        # SELL: trailing stops + rotations + VIX panic
        to_sell = set(forced_sells)
        if vix_mult == 0:
            to_sell.update(list(holdings.keys()))  # VIX panic → cash out
        else:
        ...
        monthly_journal.append({
            ...
            "vixMult":vix_mult,"regimeEvent":c["regimeEvent"]
        })
        nav_history.append({"date":date_str,"nav":final_nav})
        prev_nav = final_nav

        if (i+1)%12 == 0:
            print(f"  {date_str[:4]}: NAV Rs {final_nav/10_000_000:.2f}Cr | Tot {total:+.1f}% | VIX-mult {vix_mult}")
    ...
    print("="*70)
    print(f"CAGR:              {cagr:+.2f}%   (Honest was 25.65%, target 28-30%)")
    print(f"Nifty CAGR:        {nifty_cagr:+.2f}%")
    print(f"Alpha:             {cagr-nifty_cagr:+.2f}%")
    print(f"Max Drawdown:      {max_dd:.1f}%   (Honest was -53.7%)")
    ...
    print(f"VIX-reduced months: {vix_offs}")
    print("="*70)
    ...
    output = {
        ...
        "summary":{
            ...
            "mode":"honest-plus","exitReasons":reason_counts,
            "layers":"Honest baseline (261 stocks, historical costs, 3yr listing) + L2 15% Trailing Stop + L6 India VIX Regime",
            "assumptions":"Honest+ = Honest baseline + only 2 additional layers (trailing stop, VIX). Low overfitting risk."
        },
        "runDate":datetime.now().isoformat()
    }
    with open("backtest_honest_plus.json","w") as f:
        json.dump(output, f, indent=2, default=str)
    print(f"\nSaved backtest_honest_plus.json ({len(monthly_journal)} months, {len(completed_trades)} trades)")
